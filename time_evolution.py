...
h_init = float(raw_input("Please enter an initial value for h:> ") or "10.0") # If nothing is entered, use 10m for the initial height
...
h_array_runge = h_array.copy()
s_array_runge = s_array.copy()
...
for i in range(1,time_steps):
    ...
    h_array[i] = h_n_plus_one_euler(alpha,params.delta_f,params.delta_s,s_array[i-1],s_0,params.D,h_array[i-1],delta_t) 
   
    s_array[i] = s_n_plus_one_euler(params.delta_f,alpha,params.V,s_array[i-1],s_0,h_array[i-1],delta_t)   

    h_array_runge[i] = h_n_plus_one_runge(alpha,params.delta_f,params.delta_s,s_array_runge[i-1],s_0,params.D,h_array_runge[i-1],delta_t) 
   
    s_array_runge[i] = s_n_plus_one_runge(params.delta_f,alpha,params.V,s_array_runge[i-1],s_0,h_array_runge[i-1],delta_t)   
